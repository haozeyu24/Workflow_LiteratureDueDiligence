...
## Run-specific rationale

- The prompt is broad but workable: it names the drug class, cancer-treatment context, resistance focus, investigational and approved inhibitors, and clinical-trial/combo-trial needs.
- Keep the first pass in `pmc_learning` so open PMC full text can calibrate resistance-mechanism terms before asking for manual PDFs.
- Do not cap PubMed collection; use query refinement, declared-scope anchors, and downstream batching if the candidate set is large.
...
- `pmc_only` disables alternate open-access lookup
- `exhaustive_oa` allows Europe PMC/OpenAlex-style alternate lookup during any access phase, including early learning
