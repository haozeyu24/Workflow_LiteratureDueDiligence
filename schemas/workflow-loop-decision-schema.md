...
- `source_stage`
  Stage that produced the signal, such as `query_optimization`, `abstract_review`, `fulltext_import`, or `fulltext_review`.
- `trigger`
  Short name of the rule or observed failure mode.
- `triggered`
  Allowed: `yes`, `no`
- `action`
  Allowed: `continue`, `pause_for_user`, `build_pdf_shortlist`, `loop_to_run_guidance_reviser`, `loop_to_query_scout`, `loop_to_abstract_review`, `loop_to_fulltext_review`, `stop_blocked`
- `target_stage`
  Stage to run next.
- `rationale`
...
Loop decisions must cite structured artifacts such as `query_diagnostics.csv`, `abstract_review.csv`, `abstract_review2.csv`, `import_status.csv`, `evidence_extraction.csv`, or `pmc_mechanism_feedback.csv`.
...
`runGuidanceReviser` before `pubmedKeywordScout`, so `instruction.md`,
`topic.md`, reviewer rules, and `search_strategy.md` all reflect the same
full-text learning.
