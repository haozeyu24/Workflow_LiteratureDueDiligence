...
from workflow_db import (
    previously_rejected_pmids,
    record_collected_papers,
    record_pubmed_payloads,
    record_uri,
)
...
def require_guidance_revision_for_learned_rerun(run_dir: Path) -> list[str]:
    ...
    if not matching_rows:
        return [
            ...
            "Revise run_brief.md, generate search_strategy.md from the revised guidance plus pmc_mechanism_feedback.csv, "
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python3 tools/collection/collect_pubmed.py <run_id>")
        return 1
    ...
    constraints_path = learned_revision_path(run_dir, "revised_run_brief_path") or learned_revision_path(run_dir, "revised_constraints_path") or run_input_path(run_dir, "run_brief.md")
    ...
    pass_number = current_pass_number(run_dir)
    skipped_prior_rejections = 0
    if pass_number > 1:
        rejected_pmids = previously_rejected_pmids(run_dir)
        if rejected_pmids:
            before_count = len(pmid_to_queries)
            pmid_to_queries = {
                pmid: queries
                for pmid, queries in pmid_to_queries.items()
                if pmid not in rejected_pmids
            }
            skipped_prior_rejections = before_count - len(pmid_to_queries)
            if not pmid_to_queries:
                diagnostics_rows.extend(collection_diagnostics_rows)
                write_query_diagnostics(query_diagnostics_path, diagnostics_rows)
                print(
                    "No PubMed records remained after filtering papers rejected in earlier passes."
                )
                return 1

    rows: list[dict[str, str]] = []
    pubmed_records: list[dict[str, object]] = []
    pubmed_record_paths: dict[str, str] = {}
            ...
            rows.append(
                {
                    ...
                    "source_query": "; ".join(pmid_to_queries.get(pmid, [])),
                    "retrieval_batch": retrieval_batch,
                    "record_path": record_uri(pass_number, paper_id),
                }
            )
    ...
    record_pubmed_payloads(run_dir, pass_number, pubmed_records, pubmed_record_paths)
    print(
        f"Wrote {len(rows)} PubMed records to {manifest_path} "
        "(no PubMed collection caps allowed)"
    )
    if skipped_prior_rejections:
        print(
            f"Skipped {skipped_prior_rejections} papers previously rejected by abstract triage "
            f"before writing the pass_{pass_number:03d} manifest."
        )
    if blocked_rows:
        print(
    ...
    print(f"Wrote per-paper metadata records to SQLite; JSON mirror directory is {records_dir}")
    return 0
