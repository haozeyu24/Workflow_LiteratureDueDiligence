...
# Backward-compatible aliases for historical callers; new pass inputs use run_brief.md.
LEGACY_RUN_BRIEF_INPUTS = {
    "request.md",
    "instruction.md",
    "topic.md",
    "review_frame.md",
...
def run_input_path(run_dir: Path, filename: str) -> Path:
    ...
    if filename in LEGACY_RUN_BRIEF_INPUTS:
        current_brief = current_input_dir(run_dir) / "run_brief.md"
        if current_brief.exists():
            return current_brief
        pass1_brief = archive_path_for_pass(run_dir, 1) / "inputs" / "run_brief.md"
        if pass1_brief.exists():
            return pass1_brief
...
def canonical_has_run_data(run_dir: Path) -> bool:
    ...
    checks = [
        ...
        artifacts_dir / "abstract_triage" / "first_pass.csv",
        artifacts_dir / "abstract_triage" / "second_pass.csv",
...
def write_snapshot_manifest(run_dir: Path, target_dir: Path, pass_number: int, reason: str) -> None:
    ...
    abstract2_rows = load_csv(artifacts_dir / "abstract_triage" / "second_pass.csv")
    ...
    payload = {
        ...
        "abstract_triage_second_pass_advance_to_import": sum(
