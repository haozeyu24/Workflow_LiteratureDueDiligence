...
from pass_archive import active_artifacts_dir, current_pass_number, archive_path_for_pass, run_input_path
from workflow_db import prior_fulltext_read_state
...
def load_prior_usable_pmc_imports(run_dir: Path) -> dict[str, dict[str, str]]:
    current_pass = current_pass_number(run_dir)
    prior: dict[str, dict[str, str]] = {}
    for pass_number in range(1, current_pass):
        path = archive_path_for_pass(run_dir, pass_number) / "artifacts" / "fulltext_import" / "import_status.csv"
        if not path.exists():
            continue
        for row in load_csv(path):
            paper_id = row.get("paper_id", "").strip()
            normalized_path = row.get("normalized_path", "").strip()
            if not paper_id or not normalized_path:
                continue
            if row.get("pmc_access_status", "").strip() != "available":
                continue
            if row.get("pmc_parse_status", "").strip() != "usable":
                continue
...
def copy_prior_normalized_text(prior_row: dict[str, str], destination_dir: Path, paper_id: str) -> str:
    source_text = prior_row.get("normalized_path", "").strip()
    if not source_text:
        return ""
    source = Path(source_text)
    if not source.exists():
        return ""
    pmcid = prior_row.get("pmcid", "").strip()
    destination_name = f"{pmcid}.json" if pmcid else f"{paper_id}.json"
    destination = destination_dir / "PMC_XML" / "normalized" / destination_name
...
def update_workflow_state(run_dir: Path, manual_pdf_queue_count: int) -> None:
    state_path = active_artifacts_dir(run_dir) / "workflow_control" / "workflow_state.json"
    ...
    state["manual_pdf_queue_count"] = manual_pdf_queue_count
    state["next_action"] = "import_pmc_fulltext"
    state["reason"] = "Full-text import status was prepared from abstract-triage promotion decisions."
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python3 tools/fulltext/prepare_import_status.py <run_id>")
        return 1
    ...
    review2_path = artifacts_dir / "abstract_triage" / "second_pass.csv"
    ...
    if not review2_path.exists():
        print(f"Second abstract-triage pass table not found: {review2_path}")
        return 1
    ...
    prior_pmc_imports = load_prior_usable_pmc_imports(run_dir)
    prior_read_state = prior_fulltext_read_state(run_dir, before_pass=current_pass_number(run_dir))
    ...
    for row in selected:
        ...
        prior_pmc = prior_read_state.get(paper_id) or prior_pmc_imports.get(paper_id)
        prior_normalized_path = copy_prior_normalized_text(prior_pmc, import_dir, paper_id) if prior_pmc else ""
        if prior_normalized_path:
            reused_prior_pmc_count += 1
            import_rows.append(
                {
                    "paper_id": paper_id,
                    "pmid": pmid,
                    "pmcid": prior_pmc.get("pmcid", ""),
                    "doi": doi,
                    "title": manifest_row.get("title", ""),
                    "fulltext_access_route": prior_pmc.get("fulltext_access_route", "ncbi_pmc_xml") or "ncbi_pmc_xml",
                    "fulltext_xml_url": prior_pmc.get("fulltext_xml_url", ""),
                    "fulltext_pdf_url": prior_pmc.get("fulltext_pdf_url", ""),
                    "pmc_access_status": "available",
                    "pmc_parse_status": "usable",
                    "pdf_needed": "no",
                    "pdf_import_status": "not_attempted",
                    "normalized_path": prior_normalized_path,
                    "notes": "Copied usable PMC-normalized full text from an earlier pass into the active pass; review decisions will be recalibrated in the active pass.",
                }
            )
            continue
    ...
    update_workflow_state(run_dir, len(manual_pdf_rows))
    ...
    print(
        ...
        f"reused_prior_pmc={reused_prior_pmc_count})"
    )
