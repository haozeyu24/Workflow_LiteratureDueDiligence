...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python3 tools/pdf/build_pdf_intervention.py <run_id>")
        return 1
    ...
    if pdf_queue_count == 0:
        ...
        notes = (
            ...
            "to another collection, abstract-triage, import, and full-text-review pass before PDF access."
        )
    elif pdf_policy == "pause_for_user":
        ...
        if final_shortlist_ready:
            notes = (
                ...
                "The workflow may complete without blocking on PDFs; the shortlist remains the access action list."
            )
        else:
