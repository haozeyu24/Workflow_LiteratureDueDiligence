...
TOOL_PATHS = {
    "stage_manual_pdfs.py": WORKFLOW_ROOT / "tools" / "pdf" / "stage_manual_pdfs.py",
    "parse_pdf_fulltext.py": WORKFLOW_ROOT / "tools" / "pdf" / "parse_pdf_fulltext.py",
    "prepare_fulltext_review.py": WORKFLOW_ROOT / "tools" / "fulltext" / "prepare_fulltext_review.py",
...
def main() -> int:
    if len(sys.argv) not in {2, 3}:
        print("Usage: python3 tools/pdf/ingest_manual_pdfs.py <run_id> [downloads_dir]")
        return 1
    ...
    run_step(["python3", str(TOOL_PATHS["stage_manual_pdfs.py"]), run_id, downloads_dir])
    run_step(["python3", str(TOOL_PATHS["parse_pdf_fulltext.py"]), run_id])
    run_step(["python3", str(TOOL_PATHS["prepare_fulltext_review.py"]), run_id])
    run_step(["python3", str(TOOL_PATHS["generate_reports.py"]), run_id])
