...
from workflow_db import latest_pubmed_record_metadata
...
def load_record_metadata(run_dir: Path, records_dir: Path, paper_id: str) -> dict[str, str]:
    db_metadata = latest_pubmed_record_metadata(run_dir, paper_id)
    if db_metadata:
        return db_metadata
...
def find_match(
    ...
    record_metadata = load_record_metadata(run_dir, records_dir, paper_id)
...
def main() -> int:
    if len(sys.argv) not in {2, 3}:
        print("Usage: python3 tools/pdf/stage_manual_pdfs.py <run_id> [downloads_dir]")
        return 1
    ...
    for queue_row in queue_rows:
        match_path, match_method, match_score = find_match(queue_row, pdf_paths, used_paths, run_dir, records_dir)
