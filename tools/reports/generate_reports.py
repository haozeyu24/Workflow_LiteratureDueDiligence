...
def run_validation(run_id: str) -> tuple[bool, str]:
    result = subprocess.run(
        [sys.executable, str(WORKFLOW_ROOT / "tools" / "run" / "validate_run.py"), run_id],
        cwd=WORKFLOW_ROOT,
        text=True,
        capture_output=True,
    )
...
def infer_remaining_stages(
    ...
    if (
        ...
        or blank_count(abstract_rows, "first_pass_decision")
        or blank_count(abstract_rows, "first_pass_confidence")
        or blank_count(abstract_rows, "triage_actor")
    ):
        stages.append("abstract triage first pass")
    if (
        len(abstract2_rows) != len(paper_manifest_rows)
        or blank_count(abstract2_rows, "second_pass_decision")
        or blank_count(abstract2_rows, "second_pass_confidence")
        or blank_count(abstract2_rows, "promotion_decision")
    ):
        stages.append("abstract triage second pass")
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python3 tools/reports/generate_reports.py <run_id>")
        return 1
    ...
    if access_phase != "pmc_learning":
        for paper_id, row in abstract2_by_paper_id.items():
            ...
            if (import_row.get("normalized_path", "") or "").strip():
                continue
            rationale = (row.get("second_pass_rationale", "") or row.get("first_pass_rationale", "")).strip()
            if rationale:
                rationale += " "
    ...
    abstract_includes = sum(1 for row in abstract_rows if row.get("first_pass_decision", "") == "include")
    ...
    if papers_retrieved and (
        ...
        current_stage_lines = [
            "workflow is incomplete: abstract triage first pass is pending",
            "next stage: run `Abstract Triage Agent` on the prepared abstract review table",
        ]
        ...
        current_stage_lines = [
            "workflow is incomplete: abstract triage second pass is pending",
            "next stage: run `Abstract Triage Agent` before full-text import or PDF actions",
        ]
    if active_loops:
        current_stage_lines = [
    ...
    notes = [
        ...
        f"`Abstract Triage Agent` advanced `{advance_to_import}` papers to import and stopped `{stop_after_abstract2}` papers."
        if abstract2_rows
        else "Second abstract-triage pass has not produced promotion decisions yet.",
        f"`fullTextEvidenceAgent` currently has `{pmc_usable}` usable automated full-text papers, `{pdf_normalized}` normalized PDF papers, and `{len(queue_rows)}` papers still in the manual PDF queue."
        if import_rows
        else "`fullTextEvidenceAgent` artifacts are not available yet.",
    ...
    lines.extend(
        [
            ...
            "",
            "Do not describe this run as `done`, `complete`, `final`, or `finished` unless `python3 tools/run/completion_gate.py <run_id>` exits with code `0`.",
            "",
            "## Counts",
            "",
