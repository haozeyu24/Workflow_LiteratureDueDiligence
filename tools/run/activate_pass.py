...
ARTIFACT_TEMPLATES = [
    ...
    ("abstract_triage_first_pass_template.csv", "artifacts/abstract_triage/first_pass.csv"),
    ("abstract_triage_second_pass_template.csv", "artifacts/abstract_triage/second_pass.csv"),
...
def require_previous_pass_ready_for_learned_rerun(run_dir: Path, pass_number: int) -> list[str]:
    ...
    abstract_rows = load_csv(previous_pass / "artifacts" / "abstract_triage" / "first_pass.csv")
    abstract2_rows = load_csv(previous_pass / "artifacts" / "abstract_triage" / "second_pass.csv")
    ...
    if len(abstract_rows) != len(manifest_rows) or blank_count(abstract_rows, "first_pass_decision"):
        errors.append("previous pass first_pass.csv is incomplete")
    if (
        ...
        errors.append("previous pass second_pass.csv is incomplete")
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: python3 tools/run/activate_pass.py <run_id> <pass_number>")
        return 1
    ...
    if pass_number > 2:
        print(
            "Refusing to activate pass_003 or later: this workflow has exactly "
            "two big passes. Pass 1 is PMC learning; pass 2 is the learned final pass."
        )
        return 1
    ...
    if readiness_errors:
        ...
        for error in readiness_errors:
            print(f"- {error}")
        print(
            "Continue the active pass through abstract review, second abstract-triage pass, "
